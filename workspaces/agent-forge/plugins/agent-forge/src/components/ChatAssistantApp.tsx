...
function ChatAssistantApp() {
  ...
  const backendUrl =
    ...
    config.getOptionalString('chatbot.baseUrl') ||
    config.getString('backend.baseUrl');
  logWithContext(`backendUrl determined: ${backendUrl}`);

  if (process.env.CHATBOT_BACKEND_URL) {
    logWithContext(
      `Using CHATBOT_BACKEND_URL from environment: ${process.env.CHATBOT_BACKEND_URL}`,
    );
  } else if (config.getOptionalString('chatbot.baseUrl')) {
    logWithContext(
      `Using chatbot.baseUrl from config: ${config.getOptionalString(
        'chatbot.baseUrl',
      )}`,
    );
  } else {
  ...
  const apiKey = config.getOptionalString('chatbot.apiKey') || '';
  const agentId = config.getOptionalString('chatbot.agentId') || '';
    ...
    return (
      ...
      >
        <img
          ...
          alt="Click this Webex Logo to open Chatbot"
        />
      </Button>
    );
  }
